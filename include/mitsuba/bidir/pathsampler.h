...
public:
	 ...
	 */
	PathSampler(bool volume, ETechnique technique, const Scene *scene, Sampler *emitterSampler,
				Sampler *sensorSampler, Sampler *directSampler, int maxDepth, int rrDepth,
				bool excludeDirectIllum, bool sampleDirect, bool lightImage = true, bool hideEmitter = true,
				int minDepth = 0);
    ...
    void samplePathsPT(const Point2i &offset, PathCallbackPT callback);
	...
	Float generateSeedsPSSMLT(size_t& sampleCount, size_t seedCount,
							  bool fineGrained, const Bitmap *importanceMap,
							  std::vector<PathSeed> &seeds,
							  Float minNonZeroPathPerSeed);

  	// Make possible to change the sampler
  	void changeSampler(Sampler* sampler) {
		m_emitterSampler = sampler;
		m_sensorSampler = sampler;
		m_directSampler = sampler;

		// Just increase the number to avoid deletion
		m_emitterSampler->incRef();
		m_sensorSampler->incRef();
		m_directSampler->incRef();
	}
