...
template <typename T> struct TAABB {
	...
	inline PointType getCenter() const {
		return (max + min) / ((Scalar) 2 );
	}

  	/// Create a new AABB moved to the new center
	inline TAABB<PointType> moveTo(const PointType& newCenter) const {
		PointType center = getCenter();
		auto displacement = newCenter - center;
		TAABB<PointType> newAABB;
		for(int d=0; d<PointType::dim; ++d) {
			newAABB.min[d] = min[d] + displacement[d];
			newAABB.max[d] = max[d] + displacement[d];
		}
		return newAABB;
	}

  	/// Return the size of AABB (Tiles)
