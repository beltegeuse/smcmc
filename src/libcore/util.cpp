...
#define DONT_FORCE_COHERENT 0

void coordinateSystem(const Vector &n, Vector &b1, Vector &b2) {
#if DONT_FORCE_COHERENT
	if (std::abs(n.x) > std::abs(n.y)) {
		Float invLen = 1.0f / std::sqrt(n.x * n.x + n.z * n.z);
		b2 = Vector(n.z * invLen, 0.0f, -n.x * invLen);
	} else {
		Float invLen = 1.0f / std::sqrt(n.y * n.y + n.z * n.z);
		b2 = Vector(0.0f, n.z * invLen, -n.y * invLen);
	}
	b1 = cross(b2, n);
#else
	// a = n
...
#else
	// FIXME: The shading derivative can be wrong... [Need to be extremly carefull]
	coordinateSystem(frame.n, frame.t, frame.s);
#endif
}
