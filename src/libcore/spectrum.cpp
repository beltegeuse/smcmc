...
//Added by RWest 2017-09-28: dot product for Spectrums
//Assumes the spectrums have the same wavelength bands
//TODO: Verify if the bands match
Float Spectrum::dot(Spectrum other) const
{
	Float result = 0.0f;
	for (size_t entry = 0; entry < SPECTRUM_SAMPLES; ++entry)
	{
		Float fa = s[entry];
		Float fb = other.s[entry];
...
//Added by RWest 2017-09-28: length product for Spectrums
//Treats the Spectrum as an N-dim vector and solves for its length
Float Spectrum::length() const
{
	Float result = dot(*this);
