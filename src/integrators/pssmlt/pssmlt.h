...
struct PSSMLTConfiguration {
  ...
  bool useKelemenMutation;
	...
	EMLTStageTechniques stagedTechnique;
	...
	void dump() const {
		...
		SLog(EDebug, "   Two-stage MLT               : %s", MLTStageTechniquesName(stagedTechnique).c_str());
        if(stagedTechnique == ETwoStage) {
	...
	inline PSSMLTConfiguration(Stream *stream) {
		...
		stagedTechnique = (EMLTStageTechniques)stream->readInt();
	...
	inline void serialize(Stream *stream) const {
		...
		stream->writeInt((int)stagedTechnique);
