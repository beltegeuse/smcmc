...
public:
    PSSMLT(const Properties &props) : Integrator(props) {
        ...
        m_config.volume = props.getBoolean("volume", false);
    ...
    bool launchRendering(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        ref<ReplayableSampler> rplSampler = new ReplayableSampler(true, -1);
        ref<PathSampler> pathSampler = new PathSampler(m_config.volume, m_config.technique, scene,
                                                       rplSampler, rplSampler, rplSampler, m_config.maxDepth,
                                                       m_config.rrDepth,
        ...
        m_config.luminance = pathSampler->generateSeedsPSSMLT(luminanceSamples,
                                                              m_config.workUnits, false, m_config.importanceMap,
                                                              pathSeeds, desiredSeedPerWorkUnit);

        // Update the normalization factors in all case
        // except when we provide a refNormalization and there is no twostage / multistage used.
        if(!(m_config.refNormalization && m_config.stagedTechnique == ENoStage)) {
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        if (m_config.timeout != 0 && !timedSamples) {
            timedSamples = true;
            ...
            Log(EInfo, "Change the number of samples: "
                    SIZE_T_FMT
                    " (old: "
                    SIZE_T_FMT
        ...
        if (m_config.stagedTechnique == ETwoStage && !m_config.firstStage) {
            // Here the code is a bit messy due to the nested approach
            // used for the twoStage MLT. Indeed, if twoStage MLT is used
        ...
        // This is to check which pass we are ....
        bool firstStagePass = (m_config.stagedTechnique == ETwoStage) && m_config.firstStage;
        bool success = true;
        ...
        if (m_config.stagedTechnique != EMultiStage) {
            if (m_config.stagedTechnique == ETwoStage && firstStagePass) {
                multiStage.setLinearCount(film, sampleCount, 1);
            } else {
                // TODO: Normally we need to remove the sampleCount that we already consumed.
        ...
        for (size_t i = 0; i < multiStage.budget.size() && success; i++) {
            ...
            m_config.workUnits = 0; // Automatically scale the work units number
            success = launchRendering(scene, queue, job, sceneResID, sensorResID,
                                      samplerResID, renderingTimer, currentPass,
                                      timeFile, firstStagePass, randomNumbers.get(), film,
                                      multiStage.budget[i], accumBuffer, i);
            if (!firstStagePass && (m_config.stagedTechnique == EMultiStage))
                firstStagePass = true;
            ...
            // In case of non first stage computation...
            if (!(m_config.stagedTechnique == ETwoStage && firstStagePass)) {
