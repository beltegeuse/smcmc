 ...
 * }
 */
class MIPathReuseTracer : public SamplingIntegrator {
 protected:
    int m_maxDepth;
    int m_rrDepth;
 ...
 public:
    MIPathReuseTracer(const Properties &props)
    ...
    MIPathReuseTracer(Stream *stream, InstanceManager *manager)
		: SamplingIntegrator(stream, manager) { }

    Spectrum Li(const RayDifferential &ray,
              RadianceQueryRecord &rRec) const {
      NotImplementedError("Li for path reuse");
    }

	Float Li(const Scene *scene,
            Sampler* sampler,
            const Sensor* sensor,
            std::vector<PathReuse>& tile,
            int currID, const Vector2& randPixelPos) const {
        ...
        for(int i = 0; i < tile.size(); i++) {
            PathReuse& t = tile[i];
            if (!scene->rayIntersect(t.ray, t.its)) {
                Assert(false); // For now don't handle this case
                t.Li = Spectrum(0.f);
            }
        ...
        PathReuse& mainPixel = tile[currID];
		...
		while (depth <= m_maxDepth || m_maxDepth < 0) {
			if (!mainPixel.its.isValid()) {
                Assert(false); // Do not handle this case for now
			...
			const BSDF *bsdf = mainPixel.its.getBSDF(RayDifferential(mainPixel.ray));

			/* Possibly include emitted radiance if requested */
			if (mainPixel.its.isEmitter() && (!m_hideEmitters || scattered)) {
                Assert(false); // Not possible to see the light directly
				//Li += throughput * its.Le(-ray.d);
            }

			/* Include radiance from a subsurface scattering model if requested */
			if (mainPixel.its.hasSubsurface()) {
				Assert(false); // No BSSRDF
                //Li += throughput * its.LoSub(scene, rRec.sampler, -ray.d, rRec.depth);
            }

			if ((depth >= m_maxDepth && m_maxDepth > 0)
              || (m_strictNormals && dot(mainPixel.ray.d, mainPixel.its.geoFrame.n)
					* Frame::cosTheta(mainPixel.its.wi) >= 0)) {
			...
			DirectSamplingRecord dRec(mainPixel.its);

			if (bsdf->getType() & BSDF::ESmooth) {
				Spectrum value = scene->sampleEmitterDirect(dRec, sampler->next2D());
                value *= dRec.pdf; // Have only the throughput

				if (!value.isZero()) {
					const Emitter *emitter = static_cast<const Emitter *>(dRec.object);

					/* Allocate a record for querying the BSDF */
					BSDFSamplingRecord bRec(mainPixel.its, mainPixel.its.toLocal(dRec.d), ERadiance);

					/* Evaluate BSDF * cos(theta) */
					const Spectrum bsdfValMain = bsdf->eval(bRec);

					/* Prevent light leaks due to the use of shading normals */
					if (!bsdfValMain.isZero() &&
                       (!m_strictNormals ||
                         dot(mainPixel.its.geoFrame.n, dRec.d) * Frame::cosTheta(bRec.wo) > 0)) {

						/* Calculate prob. of having generated that direction
						   using BSDF sampling */
						Float bsdfPdfMain = (emitter->isOnSurface() && dRec.measure == ESolidAngle)
							? bsdf->pdf(bRec) : 0;

						/* Weight using the power heuristic */
						//Float weight = 1.f; //miWeight(dRec.pdf, bsdfPdf);
                        Float pdfSum = 0.0;
                        for(int i = 0; i < tile.size(); i++) {
                            PathReuse& t = tile[i];
                            if(i == currID) {
                                t.Li += (throughput * value * bsdfValMain);
                                pdfSum += 1.f;
                            } else {

                                // Check the visibility toward the light source
                                Vector shiftDir = dRec.p - t.its.p;
                                Float shiftDistance = shiftDir.length();
                                shiftDir /= shiftDistance;

                                // Recompute the bsdf and direct contribution
                                const BSDF *bsdfShift = t.its.getBSDF(RayDifferential(t.ray)); // FIXME
                                BSDFSamplingRecord bRecShift(t.its, t.its.toLocal(shiftDir), ERadiance);
                                const Spectrum bsdfValShift = bsdfShift->eval(bRecShift);

                                Ray rayShadow(t.its.p, shiftDir, Epsilon,
                                              shiftDistance*(1-ShadowEpsilon), dRec.time);
                                if (scene->rayIntersect(rayShadow) || bsdfValShift.isZero()) {
                                    continue;
                                }

                                // TODO: Do the resampling on the light position
                                Float jacobian = 1.f;
                                jacobian /= (dRec.dist*dRec.dist) / (shiftDistance*shiftDistance);
                                if(emitter->isOnSurface()) {
                                    // This is due to the area light
                                    // Other light doesn't need this change
                                    jacobian /= absDot(shiftDir, t.its.shFrame.n) / absDot(dRec.d, mainPixel.its.shFrame.n);
                                }

                                // Sensor path change
                                PositionSamplingRecord pRec(0.f);
                                DirectionSamplingRecord dRecBase(mainPixel.ray.d);
                                DirectionSamplingRecord dRecShift(t.ray.d);
                                Float baseJ = sensor->pdfDirection(dRecBase, pRec);
                                Float shiftJ = sensor->pdfDirection(dRecShift, pRec);
                                // Area domain
//                                baseJ  *= (mainPixel.its.t*mainPixel.its.t) / Frame::cosTheta(mainPixel.its.wi);
//                                shiftJ *= (t.its.t*t.its.t) / Frame::cosTheta(t.its.wi);
                                jacobian *= shiftJ / baseJ;

                                //Spectrum shiftValue = value*(dRec.dist*dRec.dist) / (shiftDistance*shiftDistance) ;
                                Spectrum shiftValue = value*jacobian;


                                // TODO: pdf doesn't change in this case
                                t.Li += (throughput * shiftValue * bsdfValShift);

                                pdfSum += 1.f; // Normally need to update this
                            }
                        }

                        misWeight += tile.size() / (pdfSum);
					}
				}
			}
...
//                Assert(false); // FIXME: no RR for now
...
//			}
    ...
    void renderBlock(const Scene *scene,
        ...
        std::vector<PathReuse> tileInfo(points.size(), PathReuse());
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            sampler->generate(offset); // TODO: Check the type of sampler

            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                Vector2 randPixelPos(sampler->next2D());
...
//                    timeSample = rRec.nextSample1D();
                ...
                for(size_t k = 0; k < points.size(); ++k) {
                    Point2i localOffset = Point2i(points[k]) + Vector2i(block->getOffset());
                    Point2 samplePos(Point2(localOffset) + randPixelPos);

                    Spectrum spec = sensor->sampleRayDifferential(
                      sensorRay, samplePos, apertureSample, timeSample);

                    // FIXME: Need to scale it
                    //sensorRay.scaleDifferential(diffScaleFactor);

                    tileInfo[k].Li = Spectrum(0.f);
                    tileInfo[k].ray = sensorRay;
                }

                Float misWeight = Li(scene, sampler, sensor, tileInfo, i, randPixelPos);

                for(size_t k = 0; k < points.size(); ++k) {
                    Point2i localOffset = Point2i(points[k]) + Vector2i(block->getOffset());
                    Point2 localSamplePos(Point2(localOffset) + randPixelPos);

                    block->put(localSamplePos, tileInfo[k].Li * misWeight, 1.f);
                    sampler->advance();
                }
            }
        }
	...
	void serialize(Stream *stream, InstanceManager *manager) const {
        Assert(false); // Need to serialize the parameters
...
MTS_IMPLEMENT_CLASS_S(MIPathReuseTracer, false, SamplingIntegrator)
MTS_EXPORT_PLUGIN(MIPathReuseTracer, "MI path reuse tracer");
MTS_NAMESPACE_END
