...
ReplayableSampler::ReplayableSampler(bool randInit, int seedMult) : Sampler(Properties()) {
	if(randInit) {
	...
	// Change the seed automatically
	// with seed mult to not play
	// the same sequence
	for(int i = 0; i < seedMult; i++) {
		m_initial = new Random(m_initial.get());
	}
