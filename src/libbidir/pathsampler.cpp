...
PathSampler::PathSampler(bool volume, ETechnique technique, const Scene *scene, Sampler *sensorSampler,
		Sampler *emitterSampler, Sampler *directSampler, int maxDepth, int rrDepth,
		bool excludeDirectIllum,  bool sampleDirect, bool lightImage, bool hideEmitter, int minDepth)
	...
	if (technique == EUnidirectional) {
		...
		if(volume) {
			...
			props.setInteger("minDepth", minDepth);
			props.setInteger("rrDepth", rrDepth);
			props.setBoolean("hideEmitters", true);
...
void PathSampler::samplePathsPT(const Point2i &offset, PathCallbackPT callback) {
    const Sensor *sensor = m_scene->getSensor();
    Point2 apertureSample(0.5f);
    Float timeSample = 0.5f;
...
Float PathSampler::generateSeedsPSSMLT(size_t& sampleCount, size_t seedCount,
								 bool fineGrained, const Bitmap *importanceMap, std::vector<PathSeed> &seeds,
								 Float minNonZeroPathPerSeed) {
	Log(EInfo, "Integrating luminance values over the image plane ("
	SIZE_T_FMT " samples)..", sampleCount);
	...
	// have 0 importance (impossible to select)
	size_t nonZeroImportance = 0;
	...
	do {
		// Double the count
		if(idSample != 0) {
			Log(EInfo, "change the sampleCount (%f < %f)", (nonZeroImportance / (Float) seedCount), minNonZeroPathPerSeed);
			size_t newSampleCount = sampleCount * (minNonZeroPathPerSeed / (nonZeroImportance / (Float) seedCount)) * 1.05;
//			Log(EInfo, "New sampling count: " SIZE_T_FMT " ("SIZE_T_FMT")", newSampleCount, sampleCount);
			sampleCount = newSampleCount;
		}

		for (; idSample<sampleCount; ++idSample) {
			...
			mean += delta / (Float) (idSample+1);
			variance += delta * (luminance - mean);
		}
	} while(minNonZeroPathPerSeed != 0 && minNonZeroPathPerSeed > (nonZeroImportance / (Float) seedCount));
	...
	Log(EInfo, " - Pourcentage non 0 importance: %f", (nonZeroImportance / (Float) sampleCount)*100);
	Log(EInfo, " - Proportion of non 0 importance per seed: %f ", nonZeroImportance / (Float) seedCount);
